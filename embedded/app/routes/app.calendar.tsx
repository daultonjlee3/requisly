...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const rows = await listCalendarPurchaseOrders(merchant.workspace.id);
  return { workspaceName: merchant.workspace.name, rows, month };
};

export default function CalendarPage() {
  const { workspaceName, rows, month } = useLoaderData<typeof loader>();

  const calendarPos = rows.map((row) => ({
    id: row.id,
  ...
  return (
    <Page
      title="Calendar"
      subtitle={`${workspaceName} · ship / arrival dates`}
      primaryAction={{
        content: "New PO",
